...
public class Generator {
    ...
    void generateNode(Node node, Writer writer, boolean first) throws IOException {
        ...
        } else if (node.getType() == NodeTypes.INTEGER || node.getType() == NodeTypes.FLOAT
                || node.getType() == NodeTypes.BOOLEAN) {
            Source<?> source = factory.getSource(node);
            writer.write("" + source.getNext());
        } else if (node.getType() == NodeTypes.NULL) {
            Source<?> source = factory.getSource(node);
            writer.write(("" + source.getNext()).equalsIgnoreCase("not null") ? "\"" + source.getNext() + "\""
                    : "" + source.getNext());
        } else {
            Source<?> source = factory.getSource(node);
            writer.write("\"" + source.getNext() + "\"");
